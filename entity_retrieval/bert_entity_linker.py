...
class BertEntityLinker:
    """
    Identify entities in a query, using bert ner model
    """
    ...
    def identify_entities(self, utterance, min_surface_score=0.3):
        """
        identify entities from utterance

        @param utterance: text to identify entities
        @param min_surface_score: min score for surface form
        @return: a sorted list of IdentifiedEntity(surface_form,entity_name, mid, mid.score, surface_form.socre, is_perfect_match)
        """
        ...
        for mention in mentions:
            ...
            # empty entities, strip or add 'the'
            ...
            # no entities retrieved, continue
            if len(entities) == 0:
                continue
