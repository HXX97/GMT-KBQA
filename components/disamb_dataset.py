...
def proc_instance(ex, linking_results, cutoff = 10):
    """
    process entity disambiguation instance
    """
    qid = str(ex['qid'])
    query = ex['question']
    ...
    for idx, entities_per_metion in enumerate(linking_results):
        entities_per_metion = entities_per_metion[:cutoff] # top 10 candidate entities
...
def read_disamb_instances_from_entity_candidates(dataset_file, candidate_file):
    """
    get entity disambugation candidates
    """
    ...
    for data in tqdm(dataset, total=len(dataset), desc='Read Exapmles'):
        qid = str(data['qid'])
        res = entity_linking_results[qid] # candidate entities
