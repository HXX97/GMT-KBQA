...
if [ "$ACTION" = "train" ]; then
    ...
    split=${3:-test}
    model_name=${4:-none}
    if [ -d "${exp_prefix}${model_name}_${split}_2hop/" ]; then
        echo "${exp_prefix}${model_name}_${split}_2hop/ already exists"
    else
        mkdir "${exp_prefix}${model_name}_${split}_2hop/"
    fi
    ...
    python relation_retrieval/cross-encoder/cross_encoder_main.py \
                            ...
                            --dataset_type WebQSP \
                            --model_save_path "${exp_prefix}${model_name}" \
                            --output_dir "${exp_prefix}${model_name}_${split}_2hop" \
                            --cache_dir hfcache/bert-base-uncased \
