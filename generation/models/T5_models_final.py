...
class T5_MultiTask_Relation_Entity_Concat_Relation20(nn.Module):
    """ 
    Relation classification + Entity disambiguation + Generation
    """
    def __init__(
        ...
        self.REL_TOKEN = ' [REL] '
        self.ENT_TOKEN = ' [ENT] '
        self.entity_sample_size = entity_sample_size # number of candidate entities
        self.relation_sample_size = relation_sample_size # number of candidate relations
        self.do_lower = do_lower
        self.add_prefix = add_prefix
        # during training, only concat true classified results(compared with golden labels); during testing, concat normally(anything prev tasks output)
        # TODO: been proved a bad practice
        self.train_concat_true = train_concat_true
    ...
    def forward(self,
        ...
        rich_textual_candidate_entities_list = list_reshape(rich_textual_candidate_entities_list, int(len(rich_textual_candidate_entities_list) / self.entity_sample_size), self.entity_sample_size) # list of [batch_size, sample_size]
        ...
        if relation_clf_labels is not None:
            ...
            relation_clf_predict_logits = torch.reshape(relation_clf_predict_logits, (input_ids_gen.size(0), self.relation_sample_size, -1))
            if self.cross_entropy_loss:
                relation_clf_predict_logits = torch.argmax(relation_clf_predict_logits, dim=2)
            else:
                relation_clf_predict_logits = relation_clf_predict_logits.squeeze(2)
            ...
            if self.train_concat_true:
                filtered_candidate_relations = filter_candidates_by_classification_results(
                    ...
                    relation_clf_predict_logits,
                    cross_entropy_loss=self.cross_entropy_loss,
                    cls_labels=torch.reshape(relation_clf_labels, (input_ids_gen.size(0), self.relation_sample_size))
                )
            else:
                filtered_candidate_relations = filter_candidates_by_classification_results(
                    textual_candidate_relations,
        ...
        if entity_clf_labels is not None:
            ...
            entity_clf_predict_logits = torch.reshape(entity_clf_predict_logits, (input_ids_gen.size(0), self.entity_sample_size, -1))
            if self.cross_entropy_loss:
                entity_clf_predict_logits = torch.argmax(entity_clf_predict_logits, dim=2)
            else:
                entity_clf_predict_logits = entity_clf_predict_logits.squeeze(2)
            ...
            if self.train_concat_true:
                filtered_candidate_entities = filter_candidates_by_classification_results(
                    ...
                    cross_entropy_loss=self.cross_entropy_loss,
                    extract_label=True,
                    cls_labels=torch.reshape(entity_clf_labels, (input_ids_gen.size(0), self.entity_sample_size))
                )
            else:
                filtered_candidate_entities = filter_candidates_by_classification_results(
                    rich_textual_candidate_entities_list,
    ...
    def inference(self,
        ...
        # [batch_size*sample_size, 1] if BCELoss, [batch_size*sample_size, 2] if CrossEntropy
        relation_clf_predict_logits = self.cls_layer(self.dropout(relation_sentence_embedding))
        relation_clf_outputs = torch.reshape(relation_clf_predict_logits, (input_ids_gen.size(0), self.relation_sample_size, -1)) # [batch_size, sample_size, 1/2]
        ...
        # [batch_size*sample_size, 1] if BCELoss, [batch_size*sample_size, 2] if CrossEntropy
        entity_clf_predict_logits = self.cls_layer(self.dropout(entity_sentence_embedding))
        entity_clf_outputs = torch.reshape(entity_clf_predict_logits,(input_ids_gen.size(0), self.entity_sample_size,-1)) # [batch_size, sample_size, 1]
