...
class T5_Multitask_Relation_Concat(nn.Module):
    ...
    def __init__(
        ...
        self.use_rich_relation = use_rich_relation
    ...
    def forward(self,
        ...
        rich_relation_clf_input_ids=None,
        rich_relation_clf_attention_mask=None,
        textual_candidate_rich_relations=None,
        do_concat=False
    ):
        # use T5 encoder to encode input_ids
        # clf_encoder_outputs: [batch_size, max_len, hidden_size]
        print('do_concat: {}'.format(do_concat))
        ...
        textual_candidate_rich_relations = list_reshape(textual_candidate_rich_relations, int(len(textual_candidate_rich_relations)/self.sample_size), self.sample_size)
        if self.use_rich_relation:
            clf_encoder_outputs = self.t5.encoder(input_ids=rich_relation_clf_input_ids, attention_mask=rich_relation_clf_attention_mask)
        else:
        ...
        if clf_labels is not None:
            ...
            if self.use_rich_relation:
                filtered_candidate_relations = filter_candidates_by_classification_results(
                    textual_candidate_rich_relations,
                    clf_predict_logits,
        ...
        for (input_src, cand_rels) in zip(textual_input_src_gen, filtered_candidate_relations):
            ...
            if do_concat:
                for rel in cand_rels:
                    if not self.use_rich_relation and normalize_relations:
